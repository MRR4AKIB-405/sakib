...
\033[1;90m══════════════════════════════════════════════════
    """)
		...
		print("")
		print("%s [%s01%s]%s CRACK RANDOM CLONE PAK  M1 %s(PRO) V1[High Speed]"%(R,G,R,Y,G))
		print("%s [%s02%s]%s CRACKRANDOM CLONE BD  M1 %s (PRO) V2[High Speed]"%(R,G,R,Y,G))
		print("%s [%s03%s]%s OWNER WHATSAPP%s (PRO) V3[Super Slow]"%(R,G,R,Y,G))
		print("%s [%s04%s]%s  EXIT PROGRAM %s(PRO) [Normal]"%(R,G,R,Y,G))
		print(GET)
		hoga = input("\n%s [?] CHOICE : "%(B))
		if hoga in ["", " "]:
			Main()
